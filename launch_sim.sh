#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
#  Full HRC Simulation Launcher  (fake UR5e hardware)
#  Usage:  ~/launch_sim.sh
#
#  Opens 5 terminals in the correct boot order:
#    T1  Fake UR5e driver
#    T2  MoveIt + RViz          ← wait for "You can start planning now!"
#    T3  HRC safety (4 sub-terminals via launch_hrc.sh)
...
terminator -e "bash -c '
    ...
    exec bash'" &
echo "[T1] Fake UR5e driver — waiting 10 s for build + controllers..."
sleep 10
