...
class HandToCollision(Node):
    ...
    def _build_collision_object(self, msg: PoseArray):
        ...
        if WEBCAM_MODE:
            Z          = FIXED_DEPTH_M
            centre_cam = np.array([u_c   / IMG_W, v_c   / IMG_H, Z])
            corner_tl  = np.array([u_min / IMG_W, v_min / IMG_H, Z])
            corner_br  = np.array([u_max / IMG_W, v_max / IMG_H, Z])
            ...
            # Webcam mode: publish directly in camera frame, skip TF
            co                 = CollisionObject()
            co.header          = Header()
